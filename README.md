...
Permanently disables the Tornado Cash governance [`0x5efda50f22d34F262c29268506C5Fa42cB56A1Ce`](https://etherscan.io/address/0x5efda50f22d34F262c29268506C5Fa42cB56A1Ce) so that _no_ future governance proposal - malicious or otherwise - can ever be submitted, voted on, or executed again, while adding `unlockAll()` so previously locked [TORN](https://etherscan.io/address/0x77777FeDdddFfC19Ff86DB637967013e6C6A116C), including TORN locked specifically to vote this in, is never trapped.

## Mechanism

The governance proxy is a [`TransparentUpgradeableProxy`](https://etherscan.io/address/0x5efda50f22d34F262c29268506C5Fa42cB56A1Ce#code) whose admin is set to itself (`LoopbackProxy`), so a `delegatecall`ed proposal can act as its own admin. [`executeProposal()`](./src/FinalGovernanceLockProposal.sol) uses that path to:
...
- `changeAdmin(0x...dEaD)` - moves the proxy admin to an address (`0x000000000000000000000000000000000000dEaD`) with no known private key, closing the loopback itself so no future call can ever reach `upgradeTo()`/`changeAdmin()` again.

Both steps are checked on-chain, in the same transaction, before the proposal considers itself done:

- the [EIP-1967](https://eips.ethereum.org/EIPS/eip-1967) implementation slot is re-read and asserted equal to `SEALED_IMPLEMENTATION`,
- the [EIP-1967](https://eips.ethereum.org/EIPS/eip-1967) admin slot is re-read and asserted equal to the burn address `0x000000000000000000000000000000000000dEaD`,
